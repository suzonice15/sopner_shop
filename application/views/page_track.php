		...
		<div class="row">
			...
			<div class="col-lg-9 col-md-9 col-12">
				...
				<form  action="<?php echo base_url()?>search" method="post">
						<div class="row row5" >
							<div class="col-sm-6 col-md-6 col-12">
								<div class="form-group">
									<input type="text"    class="form-control" name="track_id_id" value="<?php if(isset($track_id_id)) { echo $track_id_id;}?>" placeholder="Mobile Number/ Order Number"/>
								</div>
							</div>
							
							<div class="col-sm-3 col-md-3 col-12">
								<div class="form-group">
									<input type="submit" style="background-color: green"   class="btn btn-success   btn-sm form-control" value="Track order">
								</div>
							</div>
						</div>
					</form>
					...
					$curiar='';
					if(isset($track_id_id))
					{
						

						if(!empty($track_id_id))
						{

							$orders = get_result("SELECT * FROM `order_data` WHERE `order_id`='$track_id_id' or `customer_phone`='$track_id_id' and  `order_status` !='try' order by order_id  desc limit 1");
